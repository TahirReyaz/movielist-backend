import { createNewList, deleteListById, getListById, getLists, } from "../db/lists.js";
...
export const addListItem = async (req, res) => {
    try {
        ...
        if (user.lists && user.lists.length > 0) {
            const existingListIndex = user.lists.findIndex((list) => list.listtype == listtype);
            if (existingListIndex != -1) {
                const existingList = await getListById(user.lists[existingListIndex].id);
